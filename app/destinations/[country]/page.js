...
export default async function DestinationPage({ params }) {
  ...
  return (
    <main className="bg-gradient-to-br from-stone-50 via-amber-50/30 to-stone-50 min-h-screen">
      <Navbar />
      ...
      <div className="relative pt-32 pb-16 px-4 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-amber-100 text-amber-800 rounded-full text-sm font-bold uppercase tracking-wider mb-6">
      ...
      <div className="pb-24 px-4 max-w-7xl mx-auto">
        {trips.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {trips.map((trip, index) => (
              <Link
                key={trip.id}
                href={`/trips/${trip.id}`}
                className="group bg-white rounded-2xl overflow-hidden hover:shadow-2xl transition-all duration-300 border border-stone-200 hover:border-amber-200 flex flex-col hover:-translate-y-1"
              ...
              >
                <div className="relative h-72 overflow-hidden bg-stone-100">
                  ...
                  <div className="absolute top-4 right-4 z-20">
                    <div className="px-3 py-1 bg-white/95 backdrop-blur-sm rounded-full text-xs font-bold text-amber-700 shadow-lg">
                      {country}
                    </div>
                  </div>
                </div>
                ...
                <div className="p-6 flex-grow flex flex-col">
                  <h3 className="text-xl font-bold text-stone-900 mb-1 group-hover:text-amber-700 transition-colors line-clamp-2">
                    {trip.title}
                  </h3>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-stone-100">
                    ...
                    <div className="w-12 h-12 bg-amber-50 rounded-full flex items-center justify-center group-hover:bg-amber-700 group-hover:scale-110 transition-all">
                      <ArrowRight className="h-5 w-5 text-amber-700 group-hover:text-white transition-colors" />
                    </div>
                  </div>
                </div>
              </Link>
          ...
          <div className="text-center py-24 bg-white rounded-3xl border-2 border-dashed border-stone-200 shadow-sm">
            ...
            <Link 
              href="/trips" 
              className="inline-flex items-center gap-2 px-6 py-3 bg-amber-700 text-white font-bold rounded-full hover:bg-amber-800 transition-colors shadow-lg hover:shadow-xl"
            >
              Browse All Trips
